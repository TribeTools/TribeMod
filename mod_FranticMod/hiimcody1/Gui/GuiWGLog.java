...
public class GuiWGLog extends GuiScreen {
	...
	protected void keyTyped(char c, int i) {
        ...
        if (i == Keyboard.KEY_RETURN) {
            ...
            if (s.length() > 0 && Numbers.isInteger(s.trim())) {
                String s1 = message.trim();
                hist.add(message);
                //addLine(ChatColors.DarkGray+ChatColors.stripColors(">"+message));
                //GuiTools.showModMessage("Testing", text, itemId);
                WorldGuardParser entry = WorldGuard.getWgData(Integer.parseInt(ChatColors.stripColors(s1)));
                if(entry != null) {
                GuiTools.showModMessage("WG Log - "+entry.getAction(), entry.getPlayer(), entry.getItem());
                } else {
                	ModLoader.getMinecraftInstance().thePlayer.addChatMessage(ChatColors.Red+"WGLogger: Entry Not Found!");
                }
                //ChatColors.stripColors(
                handleCommand(s1);
                message = "";
                histInt = hist.size();
            }
	...
	public void drawScreen(int i, int j, float f) {
		...
		fontRenderer.drawString("["+histInt+"] Enter number for player to check: " + message + ((updateCounter / 6) % 2 != 0 ? "" : "_"), 11, height - 10, 0xffffff);
