...
import org.frantictools.franticmod.util.ArrayLib;
import org.lwjgl.input.Keyboard;
import org.lwjgl.input.Mouse;

public class GuiManuelPage extends GuiScreen {
	private final String[] lines;
	GuiScreen parent;

	int scroll = 0;

	public GuiManuelPage(GuiScreen parent, String title, String[] lines) {
		this.lines = ArrayLib.concat(new String[] { "[C]\2477\247n" + title, "" }, lines);
		this.parent = parent;
	}
	...
	public void handleMouseInput() {
		...
		if (i != 0) {
			if (i > 0) scroll = Math.max(0, scroll - 5);
			if (i < 0) scroll = Math.min(lines.length - 1, scroll + 5);
		}
	}
}
